...
- Cross-layer импорты (`core` ↔ `modules` ↔ `app`) — через алиас `@/` (настроен в `tsconfig.json` и `jest.config.js`).
- Внутри одного модуля — relative (`../services/`, `./types`).
- ESLint `eslint-plugin-boundaries` контролирует границы слоёв (см. `eslint.config.mjs`).
